 ...
 *              for the shift registers. At every clock cycle, one PB gets read. The control if the LEDs should change
 *              state is performed using a counter that counts how many clock ticks have happened.
 *
 *
...
public:
   ...
   */
  enum class LEDState { D4_ON = 0, D1_ON, D2_ON, D3_ON };

  /**
   * Constructor of the exercise logic. It it as this point where the also initializes the array with the function
   * pointers to evaluate the pushbuttons.
   *
   ...
   */
  constexpr ExerciseLogic(const ShiftRegisterController& registersController, const ShiftRegisterLED& registersLED,
                          const ShiftRegisterPB& registersPB)
    : shiftRegisterController(registersController),
      shiftRegisterLEDs(registersLED),
      shiftRegisterPBs(registersPB),
      hasStarted(false) {
  ...
  void performEvaluation() noexcept {
    ...
    if (directionFunction != nullptr) {
      ...
      // Since the initial state has all LEDs OFF, and this will never happen again (according to the exercise
      // description, State 1 - 4 always has one LED on, and it goes from State 4 to 1 without going through 0, we need
      // to track if it has already started.
      if (hasStarted == false) {
        hasStarted = true;
      }
    }
...
private:
  ...
  void evaluatePB1(IOState pb1State) {
    ...
    if (pb1State == IOState::HIGH) {
      ...
      clockTicksToChangeLEDState =
        CLK_TICKS_8_STATES_PER_SECOND;  // Change the number of clock ticks necessary to change state
    } else {
      clockTicksToChangeLEDState =
        CLK_TICKS_4_STATES_PER_SECOND;                // Change the number of clock ticks necessary to change state
  ...
  void evaluatePB2(IOState pb2State) {
    ...
    if (pb2State == IOState::HIGH) {
      ...
      clockTickCount =
        0;  // Resets the tick count, so the next time it starts again, the first LED will have the correct timing.
    }
  ...
  void reverse() {
    ...
    if (hasStarted == false) {
      // When all the LEDs are off, it first sets the mode to shift right and the make the Output QA as HIGH in that
      // shift.
      shiftRegisterLEDs.setMode(ShiftRegisterBase::Mode::SHIFT_RIGHT);
      shiftRegisterLEDs.setQAStateOnRightShift(IOState::HIGH);
      // make  a clock cycle
  ...
  DirectionFunction directionFunction =
    nullptr;                  ///< Pointer of function that controls the current direction the LEDs are moving.
  uint8_t ledStateCount = 0;  ///< Counter of the LED states

  uint8_t clockTicksToChangeLEDState =
    CLK_TICKS_4_STATES_PER_SECOND;  ///< variable used on to know currently how many clock ticks are necessary in order
                                    ///< to change LED state
...
constexpr ShiftRegisterController shiftRegisterController(
  GPIOs::getOutputHandle<IOPort::PORT_2, static_cast<uint8_t>(4)>(),
...
int main() {
  ...
  TaskHandler<5, std::chrono::milliseconds> clkTask(&evaluateAdcTask, true);
